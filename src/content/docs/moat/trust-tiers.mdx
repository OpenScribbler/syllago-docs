...
MOAT lets a manifest carry revocations from either the **registry** or the **publisher**. Syllago treats the two sources differently because they mean different things:

| Source      | Install gate         | Exit code (non-interactive) | Recovery                                                  |
|-------------|----------------------|------------------------------|-----------------------------------------------------------|
| `registry`  | Hard-block           | *(structured error, exit 1)* | Permanent (ADR 0007 G-15). Choose an alternative item.    |
