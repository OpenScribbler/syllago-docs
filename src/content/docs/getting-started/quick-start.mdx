...
> **Tip:** `syllago import --from claude-code` is an alternative that supports `--preview` and `--dry-run` flags for more control. Use `--type` and `--name` to filter what gets imported.
