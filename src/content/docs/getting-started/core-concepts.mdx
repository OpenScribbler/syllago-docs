...
For details on each provider's capabilities and file locations, see [Supported Providers](/syllago-docs/using-syllago/providers/).
...
For the full compatibility matrix and details on each type, see [Content Types](/syllago-docs/using-syllago/content-types/).
...
Content arrives in the library when you add it from a provider or import it from a file:

```bash
syllago add --from <provider>     # pull content from an installed provider
syllago import --from <provider>  # import from a provider, path, or git URL
```
...
When you've created content locally that you want to share back, you can promote it to a registry — syllago opens a PR against the registry repo:
...
For more on how these three collections work together, see [Collections](/syllago-docs/using-syllago/collections/).
...
- **Outbound** — When you install or export content to a provider, syllago converts from syllago format to the provider's native format.
...
For implementation details, see [Format Conversion](/syllago-docs/using-syllago/format-conversion/).
