---
title: Format Conversion
description: How syllago converts content between AI coding tool formats.
---

## How Conversion Works

Syllago uses a hub-and-spoke model with Claude Code as the canonical format. All conversions go through it:

```
Source Provider -> Canonical (Claude Code) -> Target Provider
```

When you run `syllago install my-skill --to cursor` or `syllago convert my-rule --to windsurf`, syllago:

1. Reads the content from your library (already in canonical format)
2. Converts to the target provider's format
3. Translates tool names, metadata, and frontmatter automatically

## Compatibility Matrix

| Content Type | Coverage | Notes |
|---|---|---|
| Rules | All 11 providers | Format varies (Markdown, MDC, YAML) but content fully preserved |
| Skills | All 11 providers | Metadata rendering varies by provider capability |
| Agents | All 11 providers | Codex uses TOML format, Kiro uses JSON (auto-converted) |
| MCP configs | 9 providers | Zed uses `context_servers` key (handled automatically) |
| Hooks | 3 providers | Claude Code, Gemini CLI, Copilot CLI. Other providers lack hook systems |
| Commands | Claude Code only | Provider-specific feature |
| Loadouts | Claude Code (v1) | Additional provider emitters planned |

## Provider-Specific Format Notes

### Cursor
Rules are converted to MDC (Markdown Component) format with `.mdc` extension. Frontmatter fields map to MDC metadata headers.

### Codex
Agent definitions use TOML format. Syllago auto-converts between TOML and Markdown frontmatter.

### Kiro
Uses JSON-based steering files. Markdown content is embedded in JSON string fields.

### Zed
MCP server configs use the `context_servers` key instead of `mcpServers`. Syllago handles this mapping automatically.

### OpenCode
Rules use YAML format. Syllago strips Markdown frontmatter and converts to OpenCode's YAML structure.

## What Gets Preserved

- **Content body** -- always preserved in full
- **Metadata** -- mapped to provider-equivalent fields where possible
- **Tool names** -- automatically translated (e.g., Claude Code's `Read` becomes Gemini's `read_file`)
- **Frontmatter** -- converted to provider-appropriate format (YAML, TOML, JSON, or embedded as prose)

When metadata can't be represented in the target format, it's either embedded as a comment or note in the content body, or reported as a warning during conversion.

## Using the Convert Command

```bash
# Convert a specific item
syllago convert my-skill --to gemini-cli

# See what would be converted (dry run output)
syllago convert my-rule --to windsurf --json
```
