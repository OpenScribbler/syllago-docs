...
Every content item in your [library](/using-syllago/collections/library/) has a `.syllago.yaml` file in its directory. This file tracks the item's identity, provenance, and [type](/using-syllago/content-types/) — syllago uses it to manage, inspect, and export content.

You rarely need to edit `.syllago.yaml` by hand. Syllago generates and maintains it automatically when you [`create`](/using-syllago/cli-reference/create/), `add`, or sync content.
...
Content created with [`syllago create`](/using-syllago/cli-reference/create/) uses `source: created` instead of the `source_type`/`source_format`/`source_hash` fields. The `added_by` field records the user who created it.
